class Helper {
    ...
    async findElById(android_id) {
        return await this.driver.$(this.androidId(android_id))
    }

    async findElByXpath(xpath) {
        return await this.driver.$(xpath)
    }

    async findElsById(android_id) {
        return await this.driver.$$(this.androidId(android_id))
    }

    async findElsByXpath(xpath) {
        return await this.driver.$$(xpath)
    }

    async findElByIdAndClick(android_id, timeout = 500) {
        const element = await this.findElById(android_id)
        await this.driver.pause(timeout)
        await element.click()
        return element
    }

    async findElByXpathAndClick(xpath, timeout = 500) {
        const element = await this.findElByXpath(xpath)
        await this.driver.pause(timeout)
        await element.click()
        return element
    }
    ...
    async slideOnElement(element, direction = 'top', threshold = 20) {
        ...
        await _calculateSlide(x, y, width, height, direction, threshold)
    }

    async slideOnScreen(direction = 'top', threshold = 100) {
        const { width, height } = await this.driver.getWindowRect()
        await _calculateSlide(0, 0, width, height, direction, threshold)
    }
    ...
    async backToActivity(activity, depth = 10, tstart = 1000, tend = 3000) {
        while (depth-- > 0) {
            const name = await client.getCurrentActivity()
            if (name == activity) {
                return name
            } else {
