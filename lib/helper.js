...
class Helper {
    ...
    async pushImages(images, clear = true) {
        const device = this.driver.options.capabilities.deviceName
        await shell(`adb -s ${device} shell mount -o remount ,rw /`)
        await shell(`adb -s ${device} shell mkdir -p /sdcard/DCIM/Camera/`)
        if (clear) {
            await shell(`adb -s ${device} shell "cd /sdcard/DCIM/Camera/ && rm -rf *.*"`)
        }
        await shell(`adb -s ${device} push ${images.map((image) => `"${image}"`).join(' ')} /sdcard/DCIM/Camera/`)
        await shell(`adb -s ${device} shell am broadcast -a android.intent.action.MEDIA_MOUNTED -d file:///storage/sdcard0/DCIM/Camera/`)
    }

    log(msg) {
        const path = process.env.MQ_LOG_PATH ? process.env.MQ_LOG_PATH : require('path').join(os.tmpdir(), this.driver.options.capabilities.deviceName)
        if (process.env.NODE_ENV === 'appium') {
            fs.appendFile(path, `${msg instanceof Error ? msg.stack : msg} ${new Date().toLocaleString()}\n`, () => { })
        }
    }
    ...
    async  waitActivitiesResumed(activities = [], timeout = 10 * 1000) {
        await this.driver.waitUntil(async () => {
            const name = await this.driver.getCurrentActivity()
